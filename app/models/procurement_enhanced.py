...
class PriceComparison(Base):
    ...
    recommended_supplier_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("suppliers.id"), nullable=True)
...
class PriceComparisonItem(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    comparison_id: Mapped[str] = mapped_column(String(36), ForeignKey("price_comparisons.id"), nullable=False)
...
class EscrowPayment(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    order_id: Mapped[str] = mapped_column(String(36), ForeignKey("procurement_orders.id"), nullable=False)
    project_id: Mapped[str] = mapped_column(String(36), ForeignKey("projects.id"), nullable=False)
    ...
    buyer_paid_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    supplier_received: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    supplier_received_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
