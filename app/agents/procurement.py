...
PRODUCT_CATEGORY_MAP = {
    ...
    "灯具": "lighting", "灯": "lighting", "吊灯": "lighting", "吸顶灯": "lighting",
    "家电": "appliance", "空调": "appliance", "冰箱": "appliance", "洗衣机": "appliance",
    "窗帘": "curtain", "百叶窗": "curtain",
    "定制家具": "custom_furniture", "家具": "custom_furniture", "沙发": "custom_furniture",
    "服务": "service", "设计服务": "service", "施工服务": "service", "安装": "service",
}

PRODUCT_UNIT_MAP = {
    "tile": "㎡", "flooring": "㎡", "paint": "桶", "cabinet": "m",
    "lighting": "个", "appliance": "台", "curtain": "m",
    "custom_furniture": "件", "service": "次",
}
...
2. 基于已有报价生成比价报告（询价/报价收集由采购流程完成）
3. 推荐最优采购方案（综合价格、交期、品质）
4. 说明采购订单与物流跟踪入口（生成/查询由采购管理功能完成）
...
请用中文回复，保持专业采购的语气。"""
    ...
    @staticmethod
    def classify_product_intent(message: str) -> str:
        """识别产品管理意图"""
        lower = message.lower()
        ...
        if any(kw in lower for kw in ["我的产品", "查看产品", "产品列表", "我的商品"]):
            return "list_my_products"
        if any(kw in lower for kw in ["发布到", "上架到", "推送到项目"]):
        ...
        if intent == "create_product":
            ...
            if info["name"] and info["category"]:
                return (
                    ...
                    f"| 名称 | {info['name']} |\n"
                    f"| 分类 | {info['category']} |\n"
                    f"| 价格 | {info['price_min'] or '—'} {info['price_max'] or ''} {info['unit'] or ''} |\n"
