...
@router.get("/nearby")
async def nearby_buildings(lng: str = "", lat: str = "", radius: int = 3000, limit: int = 10):
    """根据坐标或IP自动定位，返回周边楼盘/小区"""
    result = {"city": "", "district": "", "location": "", "buildings": [], "districts": []}
    
    # Step 1: IP 定位获取当前城市
    try:
        ip_data = await _amap_get("/ip")
        result["city"] = ip_data.get("city", "") or "北京"
        result["province"] = ip_data.get("province", "")
        if ip_data.get("rectangle"):
            rect = ip_data["rectangle"]
    ...
    # Step 2: 周边楼盘 POI 搜索
    try:
        search_params = {"keywords": "小区|楼盘|公寓", "city": city, "types": "120300|120302|120303", "offset": str(limit)}
        if lng and lat:
            search_params["location"] = f"{lng},{lat}"
            search_params["radius"] = str(radius)
        poi_data = await _amap_get("/place/around" if lng and lat else "/place/text", **search_params)
        for p in poi_data.get("pois", [])[:limit]:
    ...
    # Step 3: 行政区列表作为降级选择
    try:
        district_data = await _amap_get("/config/district", keywords=city, subdistrict="1")
        districts = district_data.get("districts", [])
        if districts:
            for d in districts[0].get("districts", [])[:10]:
                result["districts"].append({"name": d.get("name"), "center": d.get("center")})
    except Exception:
        pass
