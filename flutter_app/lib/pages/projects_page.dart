...
class _ProjectsPageState extends State<ProjectsPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final api = ApiClient();
      final data = await api.getList('/projects');
      setState(() => _projects = List<Map<String, dynamic>>.from(data as List));
    } catch (e) {
      // List API returns array directly for some routes
    }
            ...
            if (_loading)
              ...
              const Center(
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      Icon(Icons.home_work_outlined, size: 48, color: Color(0xFF5A5866)),
                      SizedBox(height: 12),
                      Text('还没有项目，点击右上角 + 创建一个吧', style: TextStyle(color: Color(0xFF5A5866))),
                    ],
                  ),
                ),
              )
