...
import 'ar_scan/ar_scan_coaching.dart'; // EnvCondition / arWarning / CoachingOverlay / EnvCoachingBanner
...
class _ARScanPageState extends State<ARScanPage> with TickerProviderStateMixin {
  ...
  PreferredSizeWidget _buildAppBar() {
    return AppBar(
      ...
      title: Row(
        ...
        children: [
          const Icon(Icons.view_in_ar, color: SuokeDesignTokens.accent, size: 20),
  ...
  Widget _buildStepIndicator() {
    ...
    return Container(
      ...
      // v1.2.10 P1：start 对齐，保证所有圆点顶部齐平；当前步骤在圆点下方渲染文字标签。
      child: Row(
        ...
        children: List.generate(steps.length, (i) {
          ...
          return Expanded(
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                _stepDot(i, isActive, isPast, color, icons[i], steps[i]),
                if (i < steps.length - 1)
                  Expanded(
                    child: Container(
                      height: 2,
  ...
  Widget _stepDot(int index, bool active, bool past, Color color, IconData icon, String label) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        AnimatedContainer(
        ...
        // v1.2.10 P1：当前步骤渲染文字标签（原为装饰性脉冲点，现改为可读标签，提升可扫读性）。
        if (active) ...[
  ...
  Widget _buildDeviceInfoGrid() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Icon(Icons.phone_android, color: SuokeDesignTokens.accent, size: 18),
            const SizedBox(width: 8),
  ...
  Widget _buildSensorStatusGrid() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Icon(Icons.sensors, color: SuokeDesignTokens.accent, size: 18),
            const SizedBox(width: 8),
  ...
  Widget _buildMethodRecommendation() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Icon(Icons.auto_fix_high, color: SuokeDesignTokens.accent, size: 18),
            const SizedBox(width: 8),
          ...
          if (_availableMethods.length > 1) ...[
            ...
            Wrap(
              spacing: 8,
              children: _availableMethods.map((m) => Semantics(
                label: '选择扫描方法: ${_methodLabel(m)}${_scanMethod == m ? '，已选中' : ''}',
                button: true,
  ...
  Widget _buildFloorplanSelector() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Icon(Icons.layers, color: SuokeDesignTokens.accent, size: 16),
            const SizedBox(width: 6),
  ...
  Widget _buildFloorplanUploadSection() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Icon(Icons.add_photo_alternate, color: SuokeDesignTokens.accent, size: 16),
            const SizedBox(width: 6),
  ...
  Widget _buildDimensionInput(RoomPreset preset) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Icon(Icons.straighten, color: SuokeDesignTokens.accent, size: 16),
            const SizedBox(width: 6),
  ...
  Widget _buildQuickFloorplanButton() {
    return Container(
      ...
      child: Column(
        children: [
          Row(children: [
            const Icon(Icons.add_chart, color: SuokeDesignTokens.accent, size: 18),
            const SizedBox(width: 8),
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: const Text('快速创建', style: TextStyle(color: SuokeDesignTokens.accent, fontSize: 12, fontWeight: FontWeight.w500)),
                ),
              ),
            ],
          ),
          ...
          children: [
            Row(children: [
              Container(
                ...
                child: const Icon(Icons.lightbulb_outline, color: SuokeDesignTokens.accent, size: 16),
              ),
              const SizedBox(width: 8),
  ...
  Widget _buildScanPreview(RoomPreset preset) {
    return AnimatedBuilder(
      ...
      builder: (_, _) {
        ...
        return Container(
          // v1.2.x UI 优化：扫描预览是扫描步骤的主视觉锚点，加高至 280 提升 AR 主导感
          height: 280,
          decoration: BoxDecoration(
            color: SuokeDesignTokens.card(context),
          ...
          child: Stack(
            ...
            children: [
              // 网格背景（静态透明度，v1.2.10 去除呼吸动效以减少视觉噪音）
              const CustomPaint(
              ...
              // v1.2.10 P0 诚实降级：移除硬编码"房间轮廓"矩形与 3 圈装饰脉冲环。
              // 二者与真实 AR 状态无关（任何房间都长一样），易让用户误以为 AR 正在"看见"房间。
              // 现仅保留：状态驱动的 reticle（颜色随追踪质量变化）+ 房间信息 + 方法标签 + 丢失遮罩。
              // 示意图标注（诚实告知用户这是预览示意，非实时相机画面）
              Positioned(
                top: 12,
                left: 12,
                child: Semantics(
                  ...
                  child: Container(
                    ...
                    child: Text('示意图',
                        style: TextStyle(color: SuokeDesignTokens.textSub(context), fontSize: 9)),
                  ),
                ),
              ),
              // 房间信息
                ...
                child: Column(
                  children: [
                    ...
                    Text('${(_roomLength * _roomWidth).toStringAsFixed(1)} ㎡',
                        style: const TextStyle(color: SuokeDesignTokens.accent, fontSize: 12)),
                  ],
                ),
              ),
              // 方法标签
                ...
                child: Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 4),
                      Text(_methodLabel(_scanMethod),
                          style: const TextStyle(color: SuokeDesignTokens.accent, fontSize: 10, fontWeight: FontWeight.w500)),
                    ],
                  ),
                ),
              ),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              _summaryChip(Icons.speed, '预估', '$_estimatedTimeMin分钟'),
            ],
          ),
          const SizedBox(height: 8),
          Row(
  ...
  Widget _buildStartScanButton(RoomPreset preset) {
    return _primaryButton('开始扫描 ${preset.name}', Icons.view_in_ar, _startScan,
        height: 56, radius: 16, fontSize: 16);
  }
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                flex: 2,
                child: _primaryButton('确认并查看结果', Icons.check_circle, () => _goToStep(_ScanStep.results)),
              ),
            ],
          ),
        ],
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            child: const Icon(Icons.tips_and_updates, color: SuokeDesignTokens.accent, size: 20),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
  ...
  /// 共享精度校准卡片（复核步骤与结果步骤复用，v1.2.x 去重收敛）
  Widget _buildCalibrationCard(String title, {bool highlight = false}) {
    return Container(
      padding: const EdgeInsets.all(16),
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            const Icon(Icons.tune, color: SuokeDesignTokens.accent, size: 16),
            const SizedBox(width: 6),
            Expanded(
              child: Text(title,
                  style: TextStyle(color: SuokeDesignTokens.text(context), fontWeight: FontWeight.w600, fontSize: 14)),
  ...
  Widget _buildInlineCalibration() {
    return _buildCalibrationCard('精度校准 (推荐)', highlight: true);
  }
  ...
  Widget _buildResultsStep() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // 完成图标（v1.2.10 P1：去除 1.0±0.05 缩放动效——几乎不可感知，属装饰性噪音）
          Center(
            child: Semantics(
              label: '扫描完成',
              child: Container(
          ...
          // 指标卡片（金色只留给关键指标，次要指标用中性色，避免强调色稀释）
          Row(
            children: [
              _resultMetricCard('房间数', '$_roomCount', Icons.home, SuokeDesignTokens.textSub(context)),
              const SizedBox(width: 10),
          ...
          Row(
            children: [
              _resultMetricCard('墙面特征', '$_wallFeatures', Icons.window, SuokeDesignTokens.textSub(context)),
              const SizedBox(width: 10),
              Expanded(
                child: Container(
                  padding: const EdgeInsets.all(16),
  ...
  Widget _buildCalibrationSection() {
    return _buildCalibrationCard('精度校准');
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          // v1.2.10 P0 诚实降级：导出功能未实现，不再伪装成可用操作。
          // 以禁用态 OutlinedButton 呈现，视觉上明确不可点击，避免用户点击后才发现"开发中"。
          Semantics(
            label: '导出测量报告，功能即将上线，当前不可用',
            button: true,
            child: SizedBox(
              width: double.infinity,
              height: 48,
              child: OutlinedButton.icon(
                onPressed: null, // 故意禁用：功能未上线
                style: OutlinedButton.styleFrom(
                  foregroundColor: SuokeDesignTokens.textSub(context),
                  side: BorderSide(color: SuokeDesignTokens.borderClr(context).withValues(alpha: 0.5)),
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                ),
                icon: Icon(Icons.file_download, size: 18, color: SuokeDesignTokens.textSub(context)),
                label: Text('导出测量报告（即将上线）',
                    style: TextStyle(fontWeight: FontWeight.w500, color: SuokeDesignTokens.textSub(context))),
              ),
            ),
          ),
          const SizedBox(height: 8),
  ...
  Widget _buildNextStepButton(String label, VoidCallback onTap) {
    return _primaryButton(label, Icons.arrow_forward, onTap);
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
              ...
              child: GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  child: Column(children: [
                    const Icon(Icons.window, color: SuokeDesignTokens.accent, size: 20),
                    const SizedBox(height: 2),
            ...
            Expanded(
              child: GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  child: Column(children: [
                    const Icon(Icons.door_sliding, color: SuokeDesignTokens.accent, size: 20),
                    const SizedBox(height: 2),
          ...
          const SizedBox(height: 10),
          _actionButton('添加门窗', Icons.add, SuokeDesignTokens.accent, _addDoorWindow),
        ],
      ),
    );
  }
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 12),
              Expanded(
                // v1.2.x 诚实降级：点位经 API 逐个落库，此处不伪造"已保存"，
                // 仅按是否有计划给出真实反馈并返回结果页。
                child: _primaryButton('完成', Icons.check, () {
                  _showSnack(_mepPlanId == null ? '尚未创建水电点位计划' : '水电点位配置完成，可返回结果页继续');
                  _goToStep(_ScanStep.results);
                }),
              ),
            ],
          ),
  ...
  Widget _buildMepCreatePlan() {
    return Container(
      ...
      child: Column(
        children: [
          const Icon(Icons.electrical_services, color: SuokeDesignTokens.accent, size: 40),
          const SizedBox(height: 12),
          Text('尚未创建水电点位计划',
  ...
  Widget _buildMepPointList() {
    if (_loadingMep) return const Center(child: CircularProgressIndicator(color: SuokeDesignTokens.accent));
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            const Spacer(),
            Text('${_mepPoints.length} 个', style: const TextStyle(color: SuokeDesignTokens.accent, fontSize: 12)),
          ]),
          const SizedBox(height: 8),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(children: [
            ...
            SizedBox(
              width: 80,
              child: _primaryButton('添加', Icons.add, _addMepPoint, height: 44, radius: 8, fontSize: 12),
            ),
          ]),
        ],
      ),
