...
async def _create_category_and_material(client: AsyncClient, token: str, cat_name: str, cat_code: str, mat_name: str, mat_sku: str, unit_price: float = 100.0, unit: str = "㎡"):
...
async def test_search_materials_by_keyword(client: AsyncClient):
    ...
    await client.post(
        "/api/materials",
        json={"category_id": data[0]["category_id"], "name": "搜索品牌物料", "sku": "SEARCH-BRAND-001", "brand": "多乐士", "unit_price": 1.0},
        headers={"Authorization": f"Bearer {token}"},
    )
...
async def test_get_project_bom_with_material_info(client: AsyncClient):
    """BOM 列表返回时应包含物料及品类信息"""
    token = await _register_and_login(client)
    cat_id, mat_id = await _create_category_and_material(client, token, "BOM详情分类", "bom_detail_cat", "BOM详情物料", "BOMD-001", 250.0)
