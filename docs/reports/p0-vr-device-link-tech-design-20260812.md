...
- ✅ **M3 完整渲染验证（2026-08-12 真机验证）**：`GaussianViewer` 端到端走通——本地自托管 `webapp/public/splats/butterfly.spz`（Spark 官方示例，4MB）后浏览器实测：**Spark 模块加载 + SplatMesh onLoad → `data-gs-status="ready"`（无 20s 超时降级）**，canvas 1172×1406 有真实渲染内容（toDataURL 50KB 非空白），拖拽环视/滚轮缩放交互可用。验证过程同时发现并修复 `list_panoramas` 漏序列化 `splat_url` 缺陷（此前前端恒 undefined → GS 永不启用）；此前 sparkjs.dev CDN 外网受限导致 .spz 加载超时触发双轨降级，符合预期降级行为（诚实降级链生效）。可观测性增强：`GaussianViewer` 容器加 `data-gs-status`（loading/ready/error）、`VirtualTour` 降级分支加 `data-pv-status="active"`，端到端断言不再依赖 console
- ✅ **M4 智能展厅最小原型（2026-08-12 落地）**：`ShowroomPage.jsx`（路由 /showroom + Shell 导航）——展厅 = 项目 VRPanorama（复用 PanoramaViewer 漫游），**展品即热点**（`HotspotCreate/Spec` 新增 `material_id`，type=exhibit）；点击展品 → Material 详情（价格/品牌/规格 + 环保认证 `/api/eco-materials/certs`）→ **一键加入 BOM**（复用 `POST /api/materials/bom` 链路）。供应商认证/实景展厅见下两条（`is_verified` + `showroom_panorama_id` 已落地）
- ✅ **M4 供应商认证状态模型（2026-08-12 落地）**：设计 4.2 的 `Supplier.is_verified` 落地——`suppliers` 表加非空布尔列（迁移 `b2d4e5f6a7b8`，默认 False 诚实标注），`SupplierResponse` 透传；**平台授予非自报**：新增 `PATCH /api/procurement/suppliers/{id}/verify`（`require_user_write` 管理员权限，认证/撤销 + 404 兜底），供应商实景展厅按此标注 pending 水印。`tests/test_procurement.py` +4 用例（默认 False 透传 / admin 授予撤销 / 非 admin 403 / 不存在 404，共 15 用例全过）
- ✅ **M4 供应商实景展厅（2026-08-12 落地）**：设计 4.2「供应商车间/样品间 → 采购商线上漫游验厂」最小实现——`suppliers` 表加 `showroom_panorama_id`（迁移 `c3e5f6a7b8c9`，FK → vr_panoramas.id，无实景内容恒 NULL 诚实标注）；`PATCH /api/procurement/suppliers/{id}`（admin 绑定展厅全景，`SupplierUpdate` schema）；`ShowroomPage` 新增「供应商实景展厅」tab——供应商卡片**认证状态诚实标注**（`is_verified` ✓ 已认证徽标 / PENDING 水印 + 「认证审核中」），有展厅全景的显示「在线验厂 · 进入实景展厅」（复用 PanoramaViewer 漫游），无实景的虚线框「待上传车间/样品间全景」；创建供应商亦可带 `showroom_panorama_id`。`tests/test_procurement.py` +3 用例（admin 绑定透传 / 非 admin 403 / 创建透传，共 18 用例全过）
- ✅ **M4 服务商作品集展厅（2026-08-12 落地）**：设计 4.3「工程队/装企口碑 → 可漫游作品集，赋能找队/接单」最小实现——`construction_crews` 表加 `showcase_panorama_id`（迁移 `d4f6a7b8c9d0`，FK → vr_panoramas.id，无作品集实景恒 NULL 诚实标注）；`PATCH /api/crews/{id}`（`require_admin` 绑定代表作全景，`ConstructionCrewUpdate` schema，创建亦可带）；`ShowroomPage` 新增「服务商作品集」tab——审核通过（review_status=approved）显示 ✓ 徽标可接单，未通过 REVIEWING 水印「审核中不可接单」，卡片含资质/评分/案例数/日单价/专长，有代表作显示「进入作品集漫游」（复用 PanoramaViewer），**发起接单**复用 `/api/crews/match` 链路（匹配评分 + 推荐理由 + 当前项目 Top 工程队）；修复 `_crew_to_response` 漏透传新字段。`tests/test_crews.py` +3 用例（admin 绑定透传 / 非 admin 403 / 创建透传，共 17 用例全过）
- ✅ **M4 装修过程透明（2026-08-12 落地）**：设计 4.3「展厅展示项目施工进度（ConstructionTask）+ 质检（QualityAssessment）时间线」——新增 `GET /api/crews/{id}/portfolio` 聚合端点（已雇佣 CrewMatch.status=hired 项目 → 施工任务阶段分布【phase 中文标签 + total/completed/in_progress/pending + 完成率进度条】+ 质检评估摘要【verdict/score/assessor/时间】），`CrewPortfolioResponse` schema 分层；`ShowroomPage` 服务商卡片新增「装修过程透明 · 施工进度/质检时间线」面板（阶段进度条 + 质检徽标，无数据诚实标注「暂无已雇佣项目的施工记录」）。`tests/test_crews.py` +3 用例（404 / 空项目诚实标注 / hired 全链路【任务阶段聚合 + 质检透传】），共 20 用例全过
- ✅ **M4 服务商付费展厅商业闭环（2026-08-12 落地）**：设计 4.3「服务商付费展厅（作品集置顶/VR 实拍权益）」最小实现——`construction_crews` 加 `owner_id`（权益归属账号，管理员平台绑定）+ `featured`（置顶标志，仅由 active 权益驱动，无权益恒 False，平台授予非自报）；新表 `crew_benefits`（权益兑换记录：crew_id/user_id/benefit_type/points_spent/expires_at/status）；`points_mall_items` 加 `benefit_type`（vip 类细分 showroom_featured / vr_photo），迁移 `e5a7b8c9d0e1` 幂等 seed 两件权益商品（「作品集置顶 · 30 天」2000 积分 /「VR 实拍权益 · 3 套」5000 积分）。**端点**：`POST /api/crews/{id}/benefits/redeem`（仅 owner 或 admin 可兑，防替他人买置顶——**复用 `points_service.redeem_item` 积分扣减 + redemption 流水**，同事务落 crew_benefits 并置 featured；防重复兑换 + 非权益商品 400 + 积分不足 400）、`GET /api/crews/{id}/benefits`（权益记录）；`list_crews` 置顶排序（featured DESC, rating DESC）+ 过期自动降级（`_sync_expired_benefits` 标 expired 并取消 featured）。**前端**：服务商卡片置顶「👑 置顶」徽标 +「展厅权益 · 置顶/VR 实拍」面板（积分商城 vip 商品展示/兑换按钮，仅归属账号可见兑换，诚实标注「暂无权益商品/仅归属账号可兑换」）。`tests/test_crews.py` +6 用例（admin 绑定 owner / 兑换置顶全链路【积分扣减 + featured + 记录 + 列表置顶】/ 非 owner 403 / 非权益商品 400 / 重复兑换 400 / 积分不足 400），共 26 用例全过
- ✅ **4.1 效果图漫游最小实现（2026-08-12 落地，非 GPU 部分）**：把 AI 效果图从静态图升级为可漫游空间的第一步——`vr_panoramas` 加 `content_source` 列（迁移 `f6a7b8c9d0e2`，**actual 实景 / effect AI 效果图**，默认 actual，诚实区分来源，前端不误当 360°）；新增 `POST /api/vr/panoramas/from-effect-render`（verify_project_access 项目校验，AI 效果图 2D 产物落库为 content_source=effect 全景，status 直接 completed 无需排队）；`VRPanoramaCreate/Response/ListItem` schema 透传。**诚实降级纪律**：AI 渲染产物为 2D 平面图（非等距柱状），`VirtualTour` 对 effect 全景走 **2D 平面预览**（img 全屏 + 徽标「效果图预览 · AI 生成非实景（2D→3D 漫游待 GPU 内容管线）」），**不伪造 360° 沉浸感**；2D→3D（.spz，DepthForge/Kairos）内容管线仍待 GPU 立项（M3 余项）。`tests/test_vr_panorama.py` +4 用例（发布 201 + content_source/status/image_url 透传 / 列表区分 effect+actual / 默认 actual 不回退 / 跨用户 403），共 16 用例全过
- ⏳ **M3 余项**：3DGS 内容管线（采集/AI 生成 → .spz，见第 4 部分 4.1 效果图漫游管线，B1 MonoDiffSplat 需 GPU/立项）
