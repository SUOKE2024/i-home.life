...
系统规模（截至 v1.15.0）：140 个 ORM 模型、80 个路由模块、112 个服务、25 个业务 Agent（21 个执行型 + 4 个商业运营型）+ 1 个总控 Orchestrator。
...
**测试基线**：全量 `pytest` 2454 passed（collect 2460 = 2454 passed + 2 skipped + 4 xfailed），为质量门禁硬约束。
