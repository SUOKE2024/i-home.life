...
// ════════════════════════════════════════════════════════
{
  ...
  const layers = [
    { t: "80 个 API 路由模块", d: "140 ORM · 112 Service", c: C.primary },
...
// ════════════════════════════════════════════════════════
{
  ...
  s.addText("索克差异化定位:不做\"效果图工具\",而是家装全流程多 Agent 自主协同 + 标准 MCP/A2A 基础设施 —— 真实业务闭环(80 路由模块/140 模型/25 Agent/112 Service)+ 开放协议 + 可编排协同层(AgentTeams)", {
...
// ════════════════════════════════════════════════════════
{
  ...
  s.addText("诚实降级原则:所有前沿功能带 feature flag + 来源标注(db / rule_based / empirical / mock),禁用硬编码假数据 —— 呼应赛题\"不伪装能力\"与白皮书空间智能(实测 80 路由模块/140 模型/25 Agent/112 Service)", {
...
// ════════════════════════════════════════════════════════
{
  ...
  s.addText("协议 Apache-2.0(与 AgentTeams 兼容)｜ 披露:LLM 商业 API(deepseek/qwen/glm/doubao)· 第三方依赖 · 数据授权边界 ｜ 已有项目基础:i-home.life(80 路由模块 / 140 模型 / 25 Agent / 112 Service)", {
