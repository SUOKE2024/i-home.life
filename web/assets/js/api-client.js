...
const ApiClient = (() => {
  ...
  // 需后端 voice_agent_orchestration_enabled=true（未启用返回 503）
  async orchestrateVoice(text, projectId = null) {
    const body = { text };
    if (projectId) body.project_id = projectId;
  ...
  async listVoiceTasks() {
    return this.request('/api/voice/orchestrate/tasks');
  },
